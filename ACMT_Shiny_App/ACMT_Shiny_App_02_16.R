...
list.of.packages <- c("shiny","ggmap", "excelR", 'rhandsontable', 'DT', 'tidycensus', 'tidyverse', 'dplyr', 'janitor', 'reshape2', 
                      'promises', 'future', 'shinythemes', 'future', 'gridExtra', 'formattable', 'shinyWidgets')
                                               ...
                                               fluidRow(style='padding-left:30px',
                                                        br(),
                                                          ...
                                                          tags$ul(
                                                            ...
                                                            tags$li("After updating, click", strong("'Load/refresh geocoded data'"), "to re-geocode the udpated addresses")
                                                          ),
                                                          em("For addresses that appear correct, but have high geocode ratings, use the mapping tool below (see step 2 below)")
                                                        ),
                                            ...
                                            conditionalPanel(
                                                 ...
                                                 fluidRow(
                                                   column(width=4,
                                                          ...
                                                          wellPanel(
                                                            tags$h4("Manually update geocode location:"),
                                                            tags$li("Click directly on the map to manually set a lat/long value"),
                                                            tags$li("Click the 'Update geocode' button below to update the lat/long value in your dataset,
                     ...
                     tabPanel('INTRODUCTION', 
                             h1('Introduction'),
                              fluidRow(style='padding-left:50px; padding-right:50px', 
                                    ...
                                    p("You will begin by uploading your dataset. If you plan to use the ACMT's built in geocoder, you can start by uploading your address data 
                                                   on the GEOCODER tab and following the steps to geocode your data."),
                                    p("If your data is already geocoded, you can proceed to the Automated COntext Measure Assembler tool, where you can upload your geocoded dataset.")
                                                    )
                                           ),
# ======= Data Pull Summaries =============================================
                      tabPanel('Data Pull Summaries', 
                                        fluidRow(style='padding:30px; padding-bottom:0px',
                                          actionButton('refreshprogress', 'Generate Data Summary')),
                               fluidRow(
...
# ======= Upload Geocoded data ============================================
                      ...
                      ), #end of Upload geocoded data tabPanel
...
# ======= NLCD ====================================================
tabPanel('> National Land Cover Database (NLCD)',
         ), #end of NLCD
# ======= ACS====================================================
tabPanel('> American Community Survey (ACS) 5-year estimates',
         ), #end of ACS
# ======= RPP ====================================================
tabPanel('> Regional Price Parity',
         ) 
                    )#end of navlist
...
# ----- Section 2: GEOCODER ------------------------------------------------
  ...
  output$example_table_preview<-renderTable({example_address})
...
## check ratings and maps
  output$dataset_geocoded2<-DT::renderDataTable(
    ...
    options = list(
      ...
      autoWidth = TRUE,
      ordering = TRUE,
      columnDefs = list(list(width = '200px', target = c(3))),
      dom = 'Bfrtip',
      buttons = c('csv', 'excel')
    ), 
    class='display'
