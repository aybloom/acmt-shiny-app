...
# ===== American Community Surevy ================================
tabPanel('>> American Community Survey', 
         #ACS settings
         fluidRow(style = 'padding:30px; padding-top:0px; padding-bottom: 0px;',
                  p(em('The ACMT has a built-in default list of ACS variable that can be pulled for a given dataset. 
                  Note that ACS variable can differ year-to-year, so if you are pulling multiple years of data, you may see NA values for certain variables for certain years')),
                  h4('Year of data:'), 
                  tags$div(style='padding-left:30px', align='left', class='multicol', 
        ...
        #          tags$div(style='padding-left:30px', align='left', class='multicol', 
        #                   pickerInput("acsVarInput","ACS Variables", choices=acsVarInputList, 
        #                               selected=all_of(acsVarInputList),
         ...
         wellPanel(style = 'padding:20px;',
                   fluidRow(
                     column(width=3,style='padding-left:30px;',
                            fluidRow(
                              #button to preview data and show progress
                              actionBttn('status_acs', 'Show/update data progress', style='simple', color='primary', size='xs')), 
                            fluidRow( 
                              div(style='font-size: 95%; color:#DC714D;font-style: italic',tableOutput('dataset_acs_message'))),),
                     column(width=2,
                            fluidRow(style = 'padding:5px;padding-top:0px',
                                     ##button to run loop to pull data
                                     actionBttn('pull_acs', 'Pull acs data', style='simple', color='success', size='xs')),
                            #column(width=2, 
                            fluidRow(style = 'padding:5px;',
                                     tags$div(style='align-center'),
                                     ##button to pause the loop
                                     actionBttn('stop_acs', 'STOP data pull', style='simple', color='danger', size='xs')), 
                     )),
                   fluidRow(style='padding:5px',
...
# ------ Section 3: ACMT ----------------------------------------------------    
  ...
  observeEvent(input$pull_acs,{
    ...
    if(!is.null(input$selectradii_acs) & file.exists('~/workspace/ACMT_Shiny_App/dataset_geocoded.csv')==TRUE){
    #load dataset_geocoded from the R environment
      
    showNotification("Importing geocoded data", type='warning', id='status_notif', duration=5)
    result_val(data.frame(Status="Loading geocoded data..."))
    ...
    result <- future({
      ...
      for(y in 1:length(years)){
        ...
        acs_vars<-unique(get_default_acs_var_list(year=year))
        
        if(file.exists('~/workspace/ACMT_Shiny_App/data_pull_measures/dataset_acs.csv')==TRUE){
          dataset_acs<-read.csv('~/workspace/ACMT_Shiny_App/data_pull_measures/dataset_acs.csv')
        }
      ...
      for(i in 1:N){
        print(paste0("Currently processing ", i, ' out of ', N, ' for the year ', year))
        #check for interrupted data process:
        id<-dataset_geocoded$id[i]
        latitude<-dataset_geocoded[dataset_geocoded$id==id,]$lat #set lat
        longitude<-dataset_geocoded[dataset_geocoded$id==id,]$long #set long
        ...
        result_val(data.frame(Status=paste("Pulling data for", id)))
          ...
          for(r in 1:length(radius_vector)){
            ...
            tryCatch({
              ...
              dataset_acs<-read.csv(paste0('~/workspace/ACMT_Shiny_App/data_pull_measures/dataset_acs.csv'))%>%dplyr::select(acs_vars[1]:radius)
              suppressMessages(
                suppressWarnings(
                  environmental_measures<-get_acmt_standard_array(long=longitude, lat=latitude, radius_meters = radius, year=year, codes_of_acs_variables_to_get = NA, 
                                                                  external_data_name_to_info_list=NULL, fill_missing_GEOID_with_zero = TRUE, set_var_list = FALSE)
                )
              )
              
              environmental_measures<-merge(environmental_measures,as.data.frame(acs_vars)%>%rename(names=acs_vars), by='names', all.y=TRUE)
              acs_measures<-environmental_measures %>% t %>% data.frame %>%row_to_names(row_number = 1)%>%mutate(id=id, year=year, radius=radius) %>% dplyr::select(all_of(acs_vars), id, year, radius )
              
              #combine 
              dataset_acs<-plyr::rbind.fill(dataset_acs, acs_measures)
